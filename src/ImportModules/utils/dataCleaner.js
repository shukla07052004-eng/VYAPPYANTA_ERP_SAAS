...
import { MARG_ERP_FIELD_MAP } from '../constants.js'
...
export function parseNumber(value, fallback = 0) {
  ...
  if (percentMatch) {
    ...
    if (Number.isFinite(parsedPercent)) return { value: parsedPercent, valid: true, raw: value }
  }
...
export function detectImportProfile(headers = []) {
  ...
  const lower = normalized.map((header) => header.toLowerCase())
  const vyaparMatches = lower.filter((header) => ['payment status', 'price/unit', 'party\'s gstin no.', 'item code', 'balance amount'].includes(header))
  if (vyaparMatches.length >= 2) return 'vyapar'
