...
import { Card, CardBody, CardHead } from '@/components/frontendUi/index.js'
import { PREVIEW_ROW_COUNT } from '@/ImportModules/constants.js'
import { processImportFile } from '@/ImportModules/core/processFile.js'
import { downloadJSON } from '@/ImportModules/core/exportJSON.js'
import { uploadImportFile } from './uploadImport.jsx'
...
export default function FileImportConverter({
  ...
  const previewColumns = previewRecords.length
    ...
    : []
  const hasCriticalErrors = (result?.criticalErrors?.length || result?.errors?.length || 0) > 0
  const hasValidationIssues = Boolean(result && ((result.warnings?.length || 0) > 0 || hasCriticalErrors))
  const canImport = Boolean(result?.payload?.records?.length && !hasCriticalErrors && !processing)

  const handleApplyImport = (importAnyway = false) => {
    if (!result || hasCriticalErrors) return
    const outcome = onImportComplete?.(result, { importAnyway })
    setAppliedReport(outcome?.stats ? { stats: outcome.stats, importAnyway } : null)
  }
      ...
      {result && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
            <ImportStat label="Status" value={hasValidationIssues ? 'Needs Review' : 'Valid'} tone={hasValidationIssues ? 'orange' : 'green'} />
          ...
          <Card>
            <CardHead
              title={hasValidationIssues ? 'This file contains validation issues.' : 'Ready to Import'}
              sub={hasValidationIssues
                ? 'Warnings do not block import. Unknown columns are ignored and only unusable rows are skipped.'
                : 'The file was mapped successfully and is ready to merge into the ERP store.'}
            />
            <CardBody style={{ display: 'grid', gap: 12 }}>
              <ImportReport result={result} appliedReport={appliedReport} />
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {hasValidationIssues && (
                  <Button
                    variant="ghost"
                ...
                <Button
                  variant="primary"
                  disabled={!canImport}
                  onClick={() => handleApplyImport(hasValidationIssues)}
                >
                  {hasValidationIssues ? 'Import Anyway' : 'Import'}
                </Button>
              </div>
            </CardBody>
          </Card>
          ...
          {(result.errors?.length > 0 || result.warnings?.length > 0) && (
            <Card>
              <CardHead title="Validation Details" sub="Critical file errors block import. Row and field warnings can be imported anyway." />
              <CardBody style={{ display: 'grid', gap: 12 }}>
...
function ImportReport({ result, appliedReport }) {
  const meta = result?.meta || {}
  const payloadMeta = result?.payload?.metadata || {}
  const records = result?.payload?.records || []
  const stats = appliedReport?.stats || null

  return (
    <div style={{ display: 'grid', gap: 10, fontSize: 13 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(145px, 1fr))', gap: 8 }}>
        <MiniReport label="Sales" value={stats?.invoices ?? 0} />
        <MiniReport label="Purchases" value={stats?.purchases ?? 0} />
        <MiniReport label="Parties" value={stats?.parties ?? 0} />
        <MiniReport label="Items" value={stats?.items ?? 0} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(145px, 1fr))', gap: 8 }}>
        <MiniReport label="Rows parsed" value={meta.totalParsedRows ?? records.length} />
        <MiniReport label="Rows skipped" value={meta.skippedRows ?? payloadMeta.skippedRows ?? 0} />
        <MiniReport label="Columns ignored" value={meta.skippedColumns ?? payloadMeta.unmappedColumns?.length ?? 0} />
        <MiniReport label="Sheet" value={meta.sheetName || 'Default'} />
      </div>
      {appliedReport && (
        <div style={{ padding: '9px 10px', borderRadius: 8, background: 'var(--green-bg)', border: '1px solid var(--green-br)', color: 'var(--green)' }}>
          Import complete. Valid data has been merged into the ERP store.
        </div>
      )}
    </div>
  )
...
function MiniReport({ label, value }) {
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, padding: '9px 10px', background: 'var(--surface)' }}>
      <div style={{ fontSize: 11, color: 'var(--ink-40)', textTransform: 'uppercase', letterSpacing: '.04em' }}>{label}</div>
      <div style={{ marginTop: 4, fontWeight: 650, color: 'var(--ink)' }}>{String(value)}</div>
    </div>
  )
}
