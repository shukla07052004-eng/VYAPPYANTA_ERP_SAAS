...
import { DEFAULT_SHORTCUTS, KEYBOARD_SETTINGS_STORAGE_KEY } from '@/hooks/useKeyboard.js'
import { PageHeader, Card, CardHead, CardBody, Input, Button } from '@/components/frontendUi'

export default function KeyboardSettingsPage({ shortcuts, onSave }) {
  const [draft, setDraft] = useState(() => ({ ...DEFAULT_SHORTCUTS, ...(shortcuts ?? {}) }))
  ...
  const saveShortcuts = () => {
    const merged = { ...DEFAULT_SHORTCUTS, ...draft }
      ...
      <Card>
        ...
        <CardBody id="keyboard-settings-form" style={{ display: 'grid', gap: 12 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            <Button variant="primary" onClick={saveShortcuts}>Save Shortcuts</Button>
          </div>
        </CardBody>
      </Card>
    </div>
